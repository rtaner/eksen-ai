...
import { validateUser, checkOwnerPermission, createSupabaseClient } from '../_shared/supabase-client.ts';
import { callGemini, parseGeminiJSON } from '../_shared/gemini.ts';
import { buildYetkinlikPrompt } from '../_shared/prompts.ts';
...
serve(async (req) => {
  ...
  try {
    ...
    // 6. Fetch closed tasks in date range (personnel or group)
    ...
    if ((!notes || notes.length === 0) && (!tasks || tasks.length === 0)) {
      return new Response(
        JSON.stringify({ error: 'Analiz için yeterli veri yok (en az 1 not veya tamamlanmış görev gerekli)' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    // 9. Format data for prompt
    ...
    // 10. Build prompt
    const prompt = buildYetkinlikPrompt(personnel.name, dateRangeFormatted, notesJSON);
